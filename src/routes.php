...
Route::group(['prefix' => $root], function() use ($controller)
{
    ...
    Route::get($category . '/thread/connect/{id}', ['uses' => $controller . '@getConnectCreateThread'])->where('id', '[A-Za-z0-9_\-]+');
	Route::post($category . '/thread/create', ['as' => 'forum.post.create.thread', 'uses' => $controller . '@postCreateThread']);
    Route::post($category . '/thread/connect/{id}', ['uses' => $controller . '@postConnectCreateThread'])->where('id', '[A-Za-z0-9_\-]+');
